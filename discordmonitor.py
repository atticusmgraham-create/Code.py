...
percent=0
weight=0
sizes=[0.104,0.142,0.13,0.137,0.143,0.025,0.087,0.118,0.016,0.079,0.064,0.139]
maximum=max(sizes)+(max(sizes)*0.6)
DISCORD_WEBHOOK_URL = ""
...
while True:
    ...
    for i in range(0,len(sizes)):
        maxd=sizes[i]+(sizes[i]*0.6)
        mind=sizes[i]-(sizes[i]*0.6)
        if(percent<maxd and percent>mind):
          motion_detected = True
    weight=percent-maximum
    ...
    if motion_detected and weight<0:
        ...
        if current_time - last_notification_time > NOTIFICATION_COOLDOWN:
            ...
            try:
                with open(final_output, "rb") as video_file:
                    payload = {"content": "🚨 **Security Notice:** Motion detected with audio! "+str(weight)}
